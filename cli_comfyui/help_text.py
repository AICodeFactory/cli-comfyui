...
Parse exit code: 0=success, 1=error, 2=result still pending/running.
"""

MAIN_EPILOG = """\
CONFIG FILE (-c / --config, default: config.json)
  JSON object fields:
    ...
    workflows_dir            string  Directory with selfhost/ and runninghub/ JSON
...
EXAMPLES
  comfyui-cli run -c config.json -w selfhost/image_flux.json -p '{"prompt":"a cat"}'
  comfyui-cli run -c config.json -w selfhost/image_flux.json -p '{}' --no-wait
...
EXAMPLES
  comfyui-cli run -c config.json -w selfhost/image_flux.json \\
    -p '{"prompt":"a cute cat","width":1024,"height":1024}'
  comfyui-cli run -c config.json -w runninghub/image_flux.json \\
    -p '{"prompt":"landscape"}' --params-file params.json
  comfyui-cli run -c config.json -w selfhost/image_flux.json -p '{}' --no-wait
"""
...
EXAMPLES
  comfyui-cli result -c config.json --prompt-id 3fa85f64-5717-4562-b3fc-2c963f66afa6
  comfyui-cli result -c config.json --prompt-id <uuid> --queue -o out.json
"""
